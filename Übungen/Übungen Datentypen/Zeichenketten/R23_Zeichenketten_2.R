...
jeweils_enthalten <- function(muster, eingaben) {
  Ergebnis <- sapply(eingaben, function(x) grepl(muster, x))
  return(unname(Ergebnis))
}
...
wort_existiert_case <- function(wort, liste) {
  return(any(grepl(wort, liste, ignore.case = TRUE)))
}
