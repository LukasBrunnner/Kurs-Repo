...
# A4 ----
mehraugen <- c(mehraugen, "nussbraun")
mehraugen <- factor(mehraugen)
...
personenMitFarbe <- function(namen, farben) {
  farben_häufigkeit <- table(farben)
  häufigste_farbe <- which.max(farben_häufigkeit)
  farbe_name <- names(farben_häufigkeit)[häufigste_farbe]
  personen <- namen[farben == farbe_name]
  return(personen)
}
...
mmmmEinkommen <- tapply(einkommen, augenfarbe, c(min(x), median(x), mean(x), max(x)))
mmmmEinkommen
# $blau
#   1500   1750   1750   2000 
# $braun
